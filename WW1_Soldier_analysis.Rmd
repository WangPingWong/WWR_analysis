...
conda install libgcc gnutls libuuid readline cmake git tmux libgfortran parallel gawk pigz rename genozip
# update conda
...
conda install -y -n $CONDA_NAME blast fastqc multiqc bioawk bbmap bowtie2 bwa bwa-mem2 cutadapt trimmomatic samtools samblaster biobambam sambamba=0.6.6 picard sequencetools mapdamage2 nextflow qualimap mawk p7zip pandas numpy genozip
# create an environment to process and analyse variants
conda create -n snp_tools samtools vcftools bcftools gatk4=4.1.9.0 freebayes snpsift stack plink admixtools admixture structure eigensoft vcfanno vt genozip # scikit-learn=0.18.2
...
mkdir -p $WORK_DIR/pbs_logs
DATA_DIR=$HOME/data/Q1369/Genomes/SNP_db/igsr_30x_GRCh38 # (copy from Qdata currently not working)
CONDA_NAME=snp_tools
mkdir -p $DATA_DIR 
cd $WORK_DIR
...
# prepare the commands to download the files
tail -n +2 $WORK_DIR/igsr_30x_GRCh38.tsv | cut -f 1 | grep -v ".tbi$" | parallel --dry-run "genozip -@ 2 -B 1 --force {} && mv {/}* $DATA_DIR/" > $WORK_DIR/get_vcfs.cmds
cd pbs_logs
GET_VCFS=$(qsub -V -J1-$(cat $WORK_DIR/get_vcfs.cmds | wc -l) -l select=1:ncpus=1:mem=8GB,walltime=5:00:00 -N get_vcf_files -v CONDA_NAME=$CONDA_NAME -vCMDS_FILE=$WORK_DIR/get_vcfs.cmds $WORK_DIR/array.pbspro | egrep -o "^[0-9]+")
