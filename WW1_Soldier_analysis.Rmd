...
#### Combined samples
Once the samples were confirmed to be of the same individual and no contaminations, we can combine the raw sequencing data and run the pipeline as a single sample.

```{bash eager_combined}
conda install -n base nextflow nf-core
DATE=`date +%d_%m_%Y`
BATCH=WWR_NF_combined
ALIGNER=bt2-local # bt2-local-N1  bwaaln  bwamem
...
REF="GRCh38"
[ ! -s "$REF_DIR/$REF.fasta" ] && pigz -cd $(find $REF_DIR -iname "*$REF*.f*a.gz") > $REF_DIR/$REF.fasta
[ ! -s "$REF_DIR/$REF.gff" ] && pigz -cd $(find $REF_DIR -iname "*$REF*.gff*.gz") > $REF_DIR/$REF.gff
#cat $REF_DIR/Dingo_MT_MH035676.1.fasta >> $REF_DIR/$REF.fasta

GENOME="$REF_DIR/$REF"
FQ_DIR=${WORK_DIR}/fastq_files
cat $FQ_DIR/UWW1_*_1.fastq.gz > $FQ_DIR/combined_WWR_1.fastq.gz
cat $FQ_DIR/UWW1_*_2.fastq.gz > $FQ_DIR/combined_WWR_2.fastq.gz
# copy input files from CloudStor
# ln -s ~/30days/data ~/aDNA # on Awoonga
# mkdir -p $FQ_DIR && cd $FQ_DIR
rclone copy -P CloudStor:aDNA/Human/WWR/fastq_files $FQ_DIR
...
# create sample table
# wget https://raw.githubusercontent.com/nf-core/test-datasets/eager/reference/TSV_template.tsv
find $FQ_DIR -maxdepth 1 -name "combined*_1*.f*q.gz" | sort | gawk '{n=split($1,a,"/"); infile2=gensub("_1\\.", "_2.", "1", $1); match(a[n], /(.+)_1\..*f.*q.gz/, sample_ids); printf "%s\t%s\t1\t4\tPE\tHuman\tdouble\thalf\t%s\t%s\tNA\n", sample_ids[1], sample_ids[1], $1, infile2}' | cat <(curl https://raw.githubusercontent.com/nf-core/test-datasets/eager/reference/TSV_template.tsv) - > $BATCH.tsv
...
nextflow-test run $HOME/etc/tools/nf-core/eager -with-singularity $NXF_SINGULARITY_CACHEDIR/nfcore-eager-2.2.2.img -params-file $BATCH.$REF.$ALIGNER.$GENOTYPER.json -c $HOME/.nextflow/qris.config
