...
# GATK Hc
GATK_HC="gatk_hc_${RUN}"
# -L MT
ls -1 UWW1*${ALIGNER}.${REF}.dedup.rg.csorted.bam | gawk -v genome=$GENOME -v ref=$REF -v ncores=$NCORES -v mapper=${ALIGNER} -v tmpdir=$TMP_DIR  '{sample_name=gensub(/\..+.dedup.rg.csorted.bam/, "", $1); printf "gatk HaplotypeCaller -R %s.fasta -I %s -O %s.HC.gvcf -ERC GVCF\n", genome, $1, sample_name}' > $GATK_HC.bash

# create a pbs file
sed -E 's/mem=[0-9]+GB,walltime=[0-9]+:00:00/mem=8GB,walltime=15:00:00/'   ${RUN}.pbspro > $GATK_HC.pbspro

GATK_ID=$( qsub -J1-$(cat ${GATK_HC}.bash | wc -l) -N ${GATK_HC:0:11} -vCMDS_FILE=${GATK_HC}.bash  ${GATK_HC}.pbspro | egrep -o "^[0-9]+") 
...
egrep -v "^GL|^KI" $GENOME.fasta.fai | cut -f1 | parallel --dry-run "gatk GenotypeGVCFs --java-options \"-Xmx12g -Xms12g\" -R $GENOME.fasta -V gendb://GenomicsDB_{} --tmp-dir $TMP_DIR -new-qual -O UWW1.all.${ALIGNER}.${REF}.{}.vcf" > GenotypeGVCFs_${BATCH}.bash

echo "gatk GenotypeGVCFs --java-options \"-Xmx12g -Xms12g\" -R $GENOME.fasta -V gendb://GenomicsDB_contigs --tmp-dir $TMP_DIR -new-qual -O UWW1.all.${ALIGNER}.${REF}.contigs.vcf" >> GenotypeGVCFs_${BATCH}.bash

GENOVCF_ID=$( qsub -J1-$(cat GenotypeGVCFs_${BATCH}.bash | wc -l) -l select=1:ncpus=1:mem=16GB,walltime=20:00:00 -N GenotypeGVCFs -W depend=afterok:$DBIMPORT_ID[] -vCMDS_FILE=GenotypeGVCFs_${BATCH}.bash  array.pbspro | egrep -o "^[0-9]+")

# can verify that all jobs finished successfully with
grep "Import completed" *.e$DBIMPORT_ID*

#DBIMPORT_ID=$( echo "cd $( pwd ) ; source ~/.bashrc; conda activate $CONDA_NAME; export TILEDB_DISABLE_FILE_LOCKING=1; gatk GenomicsDBImport --java-options \"-Xmx8g -Xms8g\" $GVCF_FILES --genomicsdb-workspace-path ${BATCH}_GenomicsDB --tmp-dir $TMP_DIR -L ${REF}_intervals.list " | qsub -V -l select=1:ncpus=4:mem=16GB,walltime=10:00:00 -N gatk_dbimport -W depend=afterok:$GATK_ID[] | egrep -o "^[0-9]+")

# call variants
#GENOVCF_ID=$(echo "cd $( pwd ) ; source ~/.bashrc; conda activate $CONDA_NAME; gatk GenotypeGVCFs --java-options \"-Xmx4g\" -R $GENOME.fasta -V gendb://${BATCH}_GenomicsDB  -new-qual -O UWW1.all.${ALIGNER}.${REF}.vcf"  | qsub -V -l select=1:ncpus=4:mem=4GB,walltime=100:00:00 -N GenotypeGVCFs -W depend=afterok:$DBIMPORT_ID | egrep -o "^[0-9]+")

# call variants from just MT 
